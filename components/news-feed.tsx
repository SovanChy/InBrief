...
export default function NewsFeed({ articles }: NewsFeedProps) {
  ...
  return (
    <div className="flex w-full h-screen">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="flex items-center justify-between p-4 border-b dark:border-gray-700 bg-white dark:bg-gray-800">
          ...
          <div className="relative w-full max-w-md mx-4">
            ...
            <Input type="text" placeholder="Search" className="pl-10 bg-gray-100 dark:bg-gray-700 border-none" />
          </div>
      ...
      <ArticleModal article={selectedArticle} isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
    </div>
  )
}
...
function ArticleCard({ image, title, timePosted, readTime, source, preview, likes, comments, tags }: ArticleCardProps) {
  const handleClick = () => {
  ...
  return (
    ...
    >      <div className="w-60 h-32 flex-shrink-0">
      ...
      <div className="flex-1">
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            {tags.map((tag, index) => (
              <Badge 
                ...
                className={`inline-flex items-center h-6 px-2 text-xs ${tag.color === "blue" ? (tag.label === "read" ? "bg-blue-900" : "bg-blue-950") : ""}`}
              >
                {tag.label}
              </Badge>
            ))}
        ...
        <div className="text-sm text-gray-500 mb-2">Source: <a href={source} className="text-blue-500 hover:underline">{source}</a></div>

        <p className="text-sm text-gray-600 dark:text-gray-300 mb-3 line-clamp-2">{preview}</p>
